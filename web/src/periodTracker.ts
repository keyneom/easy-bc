/** Local period-start log (ISO date strings) and simple length prediction for calendar-mode planning. */

const STORAGE_KEY = "easy-bc-period-starts";

export function loadPeriodStarts(): string[] {
  try {
...
export function savePeriodStarts(dates: string[]): void {
  const sorted = [...dates].sort();
  localStorage.setItem(STORAGE_KEY, JSON.stringify(sorted));
}

export function addPeriodStart(isoDate: string): void {
  const s = loadPeriodStarts();
  if (!s.includes(isoDate)) s.push(isoDate);
  savePeriodStarts(s);
}

export function removePeriodStart(isoDate: string): void {
  savePeriodStarts(loadPeriodStarts().filter((d) => d !== isoDate));
}
