...
data class PeriodRecord(
    ...
    /** Last user-visible edit; used only for deterministic cloud merges. */
...
data class UserSettingsEntity(
    ...
    /** Last portable planner edit; used only for deterministic cloud merges. */
    val updatedAt: Long = System.currentTimeMillis(),
    /** Last portable Android-preference edit; independent from planner settings. */
    val androidPreferencesUpdatedAt: Long = System.currentTimeMillis(),
)

/**
 * Flexible, app-private sync metadata. User data remains in the typed tables;
 * this table holds deletion tombstones and non-Android fields that must survive
 * a web -> Android -> web round trip.
 */
...
interface SyncMetadataDao {
    ...
    @Query("DELETE FROM sync_metadata WHERE `key` LIKE :prefix || '%'")
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * v7 -> v8: add merge timestamps plus a small metadata table for
         * tombstones and portable fields that have no Android UI yet.
         */
        val MIGRATION_7_8: Migration = object : Migration(7, 8) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE period_records ADD COLUMN updatedAt INTEGER NOT NULL DEFAULT 0")
                db.execSQL("UPDATE period_records SET updatedAt = createdAt WHERE updatedAt = 0")
                db.execSQL("ALTER TABLE day_logs ADD COLUMN updatedAt INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE user_settings ADD COLUMN updatedAt INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE user_settings ADD COLUMN androidPreferencesUpdatedAt INTEGER NOT NULL DEFAULT 0")
                db.execSQL(
