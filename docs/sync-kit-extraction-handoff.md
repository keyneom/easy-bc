# Hand-off: Extract a reusable encrypted sync kit
...
Recommended name: **`@keyneom/sync-kit`**.

This is preferable to `@keyneom/encrypted-sync` because the package will
...
- `@keyneom/snapshot-sync` — precise for EasyBC and Family Chores, but too
  narrow for Keynote's manifest/blob model.
- `@keyneom/cipher-sync` — memorable, but makes the orchestration and provider
  adapters sound secondary.
- `@keyneom/secure-sync` — clear, but "secure" is a broad claim that is harder
  to define and defend.
- `@keyneom/sync-core` — provider-neutral, but sounds like only the low-level
  layer rather than the adapters and convenience API.

Confirm the package name and repository/workspace location before scaffolding.
Unless directed otherwise, use `@keyneom/sync-kit`.
...
1. Confirm the package name and repository/workspace location.
2. Freeze deterministic v1 compatibility fixtures from EasyBC web, EasyBC
...
8. Port EasyBC web first in v1-write mode. Verify current Android/web snapshots
   still decrypt and continue syncing in both directions.
9. Port Family Chores and run its entity-merge/deletion tests.
10. Add explicit cross-app, shared-origin, and provider-isolation tests.
11. Run package-manager and consumer build matrices, then publish `v0.1.0`.
12. Add v2 readers. Change an application's writer to v2 only after every
    active consumer for that application can read v2.
13. Integrate Keynote only after defining its desktop key provider,
